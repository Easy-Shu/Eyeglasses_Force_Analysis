...
df_templeFit=pd.melt(df_data[["ID","Int_Force",'Temple_Fit_S']],id_vars=["ID","Int_Force"])
...
polyline_x =np.r_[df_templeFit_stat["Int_Force"].min()-force_interval,
                  ...
                  df_templeFit_stat["Int_Force"].max()+force_interval]# np.linspace(0.05, 0.6, 90)
...
df_lineG["x"]=np.r_[0,df_templeFit_stat["x"],df_templeFit_stat["x"].max()+1]

x_labels=(df_templeFit_stat.Int_Force*100).astype(int)/100 
x_labels2=np.repeat(None,x_labels.shape[0])
#x_labels2[[0,4,8,12,16]]=x_labels[[0,4,8,12,16]]
